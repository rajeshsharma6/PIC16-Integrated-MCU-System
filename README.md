...
* **Peripheral:** CCP2 using Timer4].
* **Function:** A single PWM signal on **RC1** controls both the power delivered to a DC fan and the brightness of the 7-segment display .
...
* **Peripheral:** CCP3 (Capture Mode) using Timer3[cite: 2, 176].
* **Logic:** Measures the exact time between rising edges of the fan's tachometer signal on **RC0**.
* **Mathematical Precision:** Uses a 24-bit binary long division algorithm ($RPM = 15,000,000 / Period$) to ensure accurate 4-digit RPM reporting without a floating-point unit .
...
* **Telemetry:** Transmits a clear string (`"RPM = 1234\r\n"`) to a PC every 1 second.
...
| File | Role |
| :--- | :--- |
| `main.asm` | Master orchestration, main loop, and variable definitions. |
| `config.inc` |System-wide I/O initialization and PPS routing. |
| `tach.inc` | Capture logic and the 24-bit/16-bit division algorithm. |
| `fanspeed.inc`|PWM setup for fan power and display dimming. |
...
| **RC0** | Fan Tachometer | CCP3 (Input)| 10k$\Omega$ Pull-up required|
...
The value **15,000,000 (0xE4E1C0)** is stored as a 24-bit dividend for the division routine.
