...
begin
    ...
    CALL    uart_test       ;Test UART Ouptut "Measuring Fan Speed" 
...
MAIN_LOOP
    ...
    CALL    timer1_run	    ; TIMER_TOGGLE (BUZZER and UART) from 0 to 1 every second
    CALL    check_alarmState; Check if ADC_4BIT >= 12
    CALL    adjust_fanspeed ; ADRESH to CCPR2H, ADRESL to CCPR2L to modify pulse width
    CALL    get_fan_speed   ; Count pulses in 1 second (RPS) and convert to RPM 
    CALL    uart_send_rpm   ; Convert RPM to BCD, Sends "RPM=XXXX\r\n" and clears TIMER_TOGGLE_UART
